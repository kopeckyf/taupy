...
from taupy import (difference_matrix, group_divergence, normalised_hamming_distance, 
                   pairwise_dispersion, number_of_groups)
import numpy as np
import pandas as pd
...
def group_divergence_leiden(simulation, *, densities=True):
    if densities:
        densities = [i.density() for i in simulation]
    ...
    if densities:
        return pd.DataFrame(list(zip(densities, divergences)), columns=["density", "divergence"])
    else:
        return divergences

def group_divergence_agglomerative(simulation, *, densities=True):
    if densities:
        densities = [i.density() for i in simulation]
...
def group_divergence_affinity_propagation(simulation, *, densities=True):
    if densities:
        densities = [i.density() for i in simulation]
